...
%Ajout d'une colone de 1 dans X
...
for i=1:m
    ...
    Jsum = Jsum + (-y(i, :)*log(h) - (1 - y(i, :))*log(1 - h));
end
J = Jsum / m;

% J'ajoute la régularisation
...
error_2 = zeros(hidden_layer_size + 1, 1);
...
for i=1:m
    ...
    h = sigmoid(Theta2*[1 ; sigmoid(Theta1*X(i, :)')]);
    error_3 = h - y(i, :)';
    error_2 = (Theta2'*error_3).*[1 ; sigmoidGradient(Theta1*X(i, :)')];
    delta_2 = delta_2 + error_3*[1 ; sigmoid(Theta1*X(i, :)')]';
    delta_1 = delta_1 + (error_2*X(i, :))(2:end, :);
